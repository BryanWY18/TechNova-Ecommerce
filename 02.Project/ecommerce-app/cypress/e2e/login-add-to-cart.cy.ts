describe('E2E: login and add a product to cart',()=>{
    ...
    it('visit/login then status code 200, add to cart then status code 200',()=>{
        ...
        cy.get('input[type="email"]').should('exist').clear().type('finux@finux.com');
        cy.get('input[type="password"]').should('exist').clear().type('finux123');
        cy.get('button[type="submit"]').should('exist').click();

        cy.wait('@loginRequest',{timeout:10000}).then((interseption)=>{
            expect(interseption.response).to.not.be.undefined;
        ...
        cy.get('[data-cy="product-card"]',{timeout:10000}).should('have.length.at.least',1).first().as('firstProduct');
